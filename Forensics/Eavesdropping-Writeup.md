...
## 🎯 Challenge Approach
 ...
 In challenge we analyze PCAP to decrypt an encrypted TLS conversation between two characters - Bob and Alice. Eve exploits an unauthenticated debug service to extract TLS session keys, which are then used to decrypt the traffic and reveal the flag.
...
- Opened in Wireshark and checked Statistics → Conversations → TCP
...
- This method was chosen because:
  - The Bob↔Alice traffic on port 4433 was TLS 1.3 encrypted
...
6. In Wireshark: Edit → Preferences → Protocols → TLS → (Pre)-Master-Secret log filename → selected `keylog.txt`
7. Filtered: `tcp.port == 4433` to view decrypted traffic 
- Examined frame 2537 (Alice → Bob) which now showed decrypted Data layer
- Hex dump revealed:
  ```
...
**[← Back to Home](../README.md)** | **[← Previous](Docker-Writeup.md)** | **[Next →](REDACTED-Writeup.md)**
